...
export const classifyWaste = onCall(async (request) => {
  ...
  if (!ai) {
    ...
    if (!apiKey) {
      ...
      throw new HttpsError("internal", "Classification service not configured. Please contact support.");
    }
  ...
  try {
    ...
    if (error.message?.includes("API key")) {
      throw new HttpsError("internal", "API configuration error. Please try again later.");
    }
